...
import { GeoOlympiadMap } from '../components/GeoOlympiadMap'
import { VirtualOlympiadCamps } from '../components/VirtualOlympiadCamps'
...
export function CalendarPage() {
  ...
  const [activeTab, setActiveTab] = useState<'calendar' | 'geo_map' | 'virtual_camps'>('calendar')
  ...
  async function rsvp(eventId: string, status: 'going' | 'maybe' | 'not_going') {
    ...
    if (current === status) {
      ...
      const labels = {
        going: t('calendar.rsvpGoing'),
        maybe: t('calendar.rsvpMaybe'),
        not_going: t('calendar.rsvpNotGoing'),
      }
      toast(labels[status])
    }
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-white p-1.5 dark:border-slate-800 dark:bg-slate-900 shadow-2xs">
        <button
          ...
          className={`flex flex-1 min-w-[130px] items-center justify-center gap-2 rounded-lg py-2.5 text-xs font-bold transition ${
            activeTab === 'calendar'
        ...
        >
          ...
          <span>{isKz ? '📅 Іс-шаралар Күнтізбесі' : '📅 Календарь Событий'}</span>
        </button>
        ...
        >
          ...
          <span>{isKz ? '📍 Олимпиада Гео-Картасы' : '📍 Гео-Карта Олимпиад'}</span>
        </button>

        <button
          type="button"
          onClick={() => setActiveTab('virtual_camps')}
          className={`flex flex-1 min-w-[130px] items-center justify-center gap-2 rounded-lg py-2.5 text-xs font-bold transition ${
            activeTab === 'virtual_camps'
              ? 'bg-[#00875A] text-white shadow-xs'
              : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'
          }`}
        >
          <Video className="h-4 w-4" />
          <span>{isKz ? '🎥 Виртуалды Кэмптер & Live' : '🎥 Виртуальные Кэмпы'}</span>
        </button>
      </div>

      {activeTab === 'geo_map' && <GeoOlympiadMap />}
      {activeTab === 'virtual_camps' && <VirtualOlympiadCamps />}
              ...
              <form className="grid gap-4 sm:grid-cols-2" onSubmit={form.handleSubmit((v) => create.mutate(v))}>
                ...
                <div>
                  ...
                  {form.formState.errors.starts_at ? (
                    <p className="mt-1 text-xs text-red-600">{form.formState.errors.starts_at.message}</p>
                  ) : null}
                </div>
                <div>
                ...
                <div className="sm:col-span-2">
                  ...
                  <input
                    ...
                    placeholder={t('calendar.locationPh')}
                ...
                <div className="flex gap-3 sm:col-span-2">
                  ...
                  <button
                    ...
                    className="rounded-lg border border-[#DFE1E6] px-4 py-2 text-sm font-semibold text-[#6B778C] hover:bg-[#F4F5F7] transition"
                  >
                    {t('common.cancel')}
                  </button>
                </div>
              ...
              <div className="space-y-3">
                {upcomingEvents.map((e) => {
                  ...
                  return (
                    <div
                      key={e.id}
                      className="flex items-start gap-4 rounded-xl border border-[#eef1f4] p-4 hover:border-[#0052CC]/20 transition"
                    >
                      <div className="flex h-12 w-12 shrink-0 flex-col items-center justify-center rounded-xl bg-gradient-to-br from-[#E3FCEF] to-[#DEEBFF] text-center">
                        <span className="text-xs font-bold text-[#00875A]">
                          {format(new Date(e.starts_at), 'dd', { locale: dfLocale })}
                        </span>
                        <span className="text-[9px] font-semibold uppercase text-[#6B778C]">
                          {format(new Date(e.starts_at), 'MMM', { locale: dfLocale })}
                        </span>
                      </div>
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-0.5 flex flex-wrap gap-2 text-xs text-[#6B778C]">
                          ...
                          {e.is_online ? (
                            <span className="rounded-full bg-[#DEEBFF] px-1.5 font-semibold text-[#0052CC]">
                              {t('calendar.onlineCheck')}
                            </span>
                          ) : null}
                        ...
                        {e.owner_id !== userId ? (
                          <div className="mt-2 flex gap-2">
                            {(['going', 'maybe', 'not_going'] as const).map((s) => {
                              const labels = {
                                going: t('calendar.rsvpGoing'),
                                maybe: t('calendar.rsvpMaybeShort'),
                                not_going: t('calendar.rsvpNotGoing'),
                              }
                              return (
                                <button
                          ...
                          <span className="mt-1 inline-block text-[10px] text-[#0052CC] font-semibold">
                            {t('calendar.yourEvent')}
                          </span>
                        )}
                      </div>
                      {e.owner_id === userId ? (
                        <button
                          type="button"
                          onClick={() => void removeEvent(e.id, e.title)}
        ...
        >
          <div className="ushqn-card w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 space-y-2 text-sm text-[#6B778C]">
              <p>📅 {format(new Date(selectedEvent.starts_at), 'PPp', { locale: dfLocale })}</p>
              ...
              {selectedEvent.description ? (
                <p className="mt-2 text-[#172B4D]">{selectedEvent.description}</p>
              ) : null}
            </div>
            {(() => {
                ...
                {(['going', 'maybe', 'not_going'] as const).map((s) => {
                  ...
                  return (
                    <button
                      key={s}
                      type="button"
                      onClick={() => void rsvp(selectedEvent.id, s)}
                      className={`flex-1 rounded-lg border py-2 text-xs font-semibold transition ${
                        myRsvp === s
                          ? 'border-[#0052CC] bg-[#0052CC] text-white'
                          : 'border-[#DFE1E6] text-[#172B4D] hover:border-[#0052CC]'
                      }`}
                    >
                      {labels[s]}
                    </button>
